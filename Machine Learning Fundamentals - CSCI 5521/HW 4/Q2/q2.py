...
for i, min_entropy in enumerate(candidate_min_entropy):
    ...
    cur_train_accuracy = np.count_nonzero(
        predictions_train.reshape(-1) == train_y.reshape(-1)
    ) / len(train_x)
    cur_valid_accuracy = np.count_nonzero(
        predictions_val.reshape(-1) == valid_y.reshape(-1)
    ) / len(valid_x)
    valid_accuracy.append(cur_valid_accuracy)
    print(
        "Training/validation accuracy for minimum node entropy %f is %.3f / %.3f"
        % (candidate_min_entropy[i], cur_train_accuracy, cur_valid_accuracy)
    )
