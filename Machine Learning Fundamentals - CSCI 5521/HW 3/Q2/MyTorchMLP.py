...
class OptDigits(Dataset):
    ...
    def __init__(self, type="train"):
        ...
        data = np.genfromtxt("/content/drive/MyDrive/Courses/MachineLearningFundamentals/hw3 - machine learning/hw3_programming/Q2/optdigits_{}.txt".format(self.type), delimiter=",")
...
class Net(nn.Module):
    def __init__(self, hidden_layer_num=2, hidden_unit_num_list=(256, 128), activation_function="Sigmoid", dropout_rate=0.5):
        ...
        for i in range(0, self.hidden_layer_num):
          self.deep_nn.add_module(f'ff{i}', nn.Linear(input_size, self.hidden_unit_num_list[i]))
          if self.activation_function == "Sigmoid":
            self.deep_nn.add_module(f'actiavtion{i}', nn.Sigmoid())
          elif self.activation_function == "Tanh":
            self.deep_nn.add_module(f'actiavtion{i}', nn.Tanh())
          elif self.activation_function == "Relu":
            self.deep_nn.add_module(f'actiavtion{i}', nn.ReLU())
          input_size = self.hidden_unit_num_list[i]
        self.deep_nn.add_module(f'classifier', nn.Linear(self.hidden_unit_num_list[-1], 10))
...
class MyPyTorchMLP(nn.Module):
    def __init__(self, batch_size=32, lr=1e-1, epoch=200, hidden_layer_num=2, hidden_unit_num_list=(128, 64),
                 activation_function="Relu", dropout_rate=0.5, check_continue_epoch=10, regularisation = "L2", logging_on=False):
        super().__init__()
        ...
        for epoch in range(self.epoch):  # loop over the dataset multiple times
            ...
            for i, data in enumerate(self.train_loader, 0):
                ...
                if self.regularisation == "L2":
                  optimizer = optim.SGD(self.parameters(), lr=0.01, weight_decay=0.01)
